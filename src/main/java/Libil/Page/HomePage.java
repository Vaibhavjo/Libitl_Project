	...
	public void serchClients(String ClientName) {
		...
		for (int i = 2; i <= PAGE_NUMBER.size() - 1; i++) {
			...
			for (int j = 0; j < All_Clients_list.size(); j++) {
				...
				// driver.findElement(By.xpath("(//a[text()='Generate Report'])[" + j + "]"));
...
//
//			}
				for (int a = 1; a <= GenerateReport.size(); a++) {
					WebElement gReport = driver.findElement(By.xpath("(//a[text()='Generate Report'])[" + a + "]"));
